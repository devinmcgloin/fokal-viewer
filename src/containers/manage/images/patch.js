...
class ManageImage extends Component {
  ...
  render = () => {
    ...
    if (this.state.status === 'success')
      alert = (
        <SuccessAlert message="Image Attributes Changed." active={this.state.status !== ''} />
      );
    ...
    return (
      <div id={this.state.image.id} className="sans-serif dib pv3 mv4 w-100">
        {alert}
        <div className="fl w-100 w-50-m w-50-l dt pa2">
          <img src={this.state.image.src_links.medium} alt="" className="dtc v-mid" />
        </div>
        <Tabs className={'fl mw6 w-100 w-50-m w-50-l'}>
          ...
          <TabPanel>
            <Exif image={this.props.image} onSubmit={this.handleSubmit} />
          </TabPanel>
          <TabPanel>
            <Geo image={this.props.image} onSubmit={this.handleSubmit} />
          </TabPanel>
          <TabPanel>
            <Tags image={this.state.image} onSubmit={this.handleSubmit} />
          </TabPanel>
          <TabPanel>
            <Gear image={this.state.image} onSubmit={this.handleSubmit} />
          </TabPanel>

          <TabPanel>
            <div className="fl ph2 pr0-ns pl3-ns dib w-100">
              <p className="measure f7">
                Deleting an image cannot be undone. All stats, favorites, tags will loose access to
      ...
      <div className="fl ph2 pr0-ns pl3-ns dib w-100">
        <form onSubmit={this.handleSubmit}>
          <div className="measure">
            ...
            <small id={'tags-desc'} className="f6 black-60 db mb2 mt1">
              Tags appear on the image page and are available for searching.
            </small>
          </div>
    ...
    return (
      <div>
        {this.state.images.map((image, indx) => (
          <ManageImage key={image.id} image={image} indx={indx} onDelete={this.handleDelete} />
        ))}
      </div>
    );
  }
