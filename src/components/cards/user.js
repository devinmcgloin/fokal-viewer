...
const UserTitleCard = ({user}) => <article className="center bg-white br2 pa3 pa4-ns ba b--black-10">
    <div className="tc">
...
const UserStatsCard = ({title, value, background}) => <div className="sans-serif">
    <div className="br2 shadow-5" style={{
...
const UserCard = (props) => {
    let content;
    if (props.horizontal) {
        content = <div className={'cf'}>
            <div className={'fl w-40 ph3'}>
                <img src={props.user.avatar_links.thumb} className="br-100 h4 w4 ba b--black-05"
                     title=""/>
            </div>
            <div className={'fl w-60 ph3'}>
                <h1 className="f3 mb2 black">{props.user.name}</h1>
                <h2 className="f5 fw4 gray mt0 black">{props.user.location || props.user.id}</h2>
            </div>
        </div>
    } else {
        content = <div>
            <div className="tc">
                <img src={props.user.avatar_links.thumb} className="br-100 h4 w4 dib ba b--black-05"
                     title=""/>
                <h1 className="f3 mb2 black">{props.user.name}</h1>
                <h2 className="f5 fw4 gray mt0 black">{props.user.location || props.user.id}</h2>
            </div>
            <div className="dib">
                {props.images.slice(0, 3).map(i => <Link className="fl w-33 pa1" key={i.id} to={'/i/' + i.id}>
                    <img src={i.src_links.thumb} className="br2"/>
                </Link>)}
            </div>
        </div>
    }
    return <article className="sans-serif bg-white br2 ba b--black-10 shadow-5">
