...
const UserTitleCard = ({ user }) => (
  <article className="center bg-white br2 pa3 pa4-ns ba b--black-10">
    <div className="tc">
      <img alt={`avatar-${user.id}`} src={user.avatar_links.thumb} className="br-100 h4 w4 dib" />
    ...
    content = (
      <div className={'cf'}>
        ...
        <div className={'fl w-60 ph3'}>
          ...
          <h2 className="f5 fw4 gray mt0 black">{user.location || user.id}</h2>
        </div>
      </div>
    );
  } else {
      ...
      <div>
        ...
        <div className="dib">
          {images.slice(0, 3).map(i => (
            <Link className="fl w-33 pa1" key={i.id} to={'/i/' + i.id}>
              <img alt={i.id} src={i.src_links.thumb} className="br2" />
            </Link>
          ))}
        </div>
      </div>
