...
const tagEntities = (text, callback) => {
  ...
  fetch('https://language.googleapis.com/v1/documents:analyzeEntities?key=' + key, {
    method: 'POST',
    body: JSON.stringify({
      document: document,
      encodingType: 'UTF8'
    ...
    .then(results => {
      ...
      entities.forEach(entity => {
        if (entity.mentions[0].type === 'PROPER' && entity.type === 'LOCATION') {
          loc = entity.name;
        } else {
      ...
      if (loc) {
        fetch('https://maps.googleapis.com/maps/api/geocode/json?address=' + loc + '&key=' + key)
          .then(resp => resp.json())
          .then(resp => {
